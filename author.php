...
if(isset($_GET['author_id'])){
    ...
    $result_cat=mysqli_query($connection,$query_cat) or die("author query failed");
    $row_cat=mysqli_fetch_assoc(($result_cat));

?>

      <h2 class="page-heading"><?php echo $row_cat['username'];?></h2>
    ...
    $query="SELECT post.post_id,post.post_img, post.title,post.category, post.description, post.post_date, category.category_name,user.username,post.author FROM post 
...
#-----------------------------------pagination---------
  ...
  if(mysqli_num_rows($result2)) {
      ...
      if($page_number>1){
        echo '<li><a href="author.php?cid ='.$author_id.'&page='.($page_number-1).'">prev</a></li>';
      }
